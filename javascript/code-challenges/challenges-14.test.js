...
const screenForNames = (arr) => {
  ...
  let notAllowedRegex = /^(mr.  )/gi;
  let Arr = arr.filter(item => regex.test(item) && !notAllowedRegex.test(item) ? true : false);
  return Arr;
}
...
const toTitleCase = (arr) => {
  // Solution code here...
  let Arr = arr.map(item => item.charAt(0).toUpperCase() + item.slice(1));
  return Arr;
};
...
let biggerThanLuke = (arr) => {
  // Solution code here...
  let biggerMe = [];
  arr.length === 0 ? [] : arr.forEach(element => element.mass > 77 ? biggerMe.push(element.name) : false);
  return biggerMe.join(' - ');
};
...
const isSecure = (url) => {
  // Solution code here...
  let testSecure = url.includes('https://') ? true : false;
  return testSecure;
};
...
------------------------------------------------------------------------------------------------ */
const testRowCol = (arr, row1, col1, row2, col2, row3, col3) => {
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  let win = false;
  testRowCol(board, 0, 0, 1, 0, 2, 0) ? win = true : win;
  testRowCol(board, 0, 1, 1, 1, 2, 1) ? win = true : win;
  testRowCol(board, 0, 2, 1, 2, 2, 2) ? win = true : win;
  testRowCol(board, 0, 0, 0, 1, 0, 2) ? win = true : win;
  testRowCol(board, 1, 0, 1, 1, 1, 2) ? win = true : win;
  testRowCol(board, 2, 0, 2, 1, 2, 2) ? win = true : win;
  testRowCol(board, 0, 0, 1, 1, 2, 2) ? win = true : win;
  testRowCol(board, 0, 2, 1, 1, 2, 0) ? win = true : win;
