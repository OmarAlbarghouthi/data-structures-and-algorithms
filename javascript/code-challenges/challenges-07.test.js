...
const howMuchPencil = (str) => {
  ...
  for (let y = 0; y <= str.length; y++) {
    result.push(str.split('').slice(y, str.length).join(''));
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  // Solution code here...
  let Array = [];
  recipe.ingredients.map(item => Array.push(item.slice(item.indexOf(' ') + 1, item.length)));
  Array.map(item => result.push(item.slice(item.indexOf(' ') + 1, item.length)));
  return result;
};
