...
const maxInArray = (arr) => {
  // Solution code here...
  let Arr = arr.reduce((idx, item) => idx < item ? idx = item : idx, 0);
  return Arr;
};
...
const getHouses = (arr) => {
  let houses = [];
  // Solution code here...
  houses = Object.values(arr).map(item => item.house);
  return houses;
};
/*------------------------------------------------------------------------------------------------
CHALLENGE 6
...
const hasChildrenValues = (arr, character) => {
  ...
  let Test = false;
  Object.values(arr).forEach(element => element.children && element.name.includes(character) ? Test = true : Test );
  return Test;
};
