...
const firstLetters = (arr) => {
  // Solution code here...
  let Arr = arr.map(item => item.charAt(0));
  return Arr;
};
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const findHappiness = (arr) => {
  // Solution code here...
  let Arr = arr.filter(item => item.includes(':)') ? true : false);
  return Arr;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let Arr = arr.map(item => `${item.substring(1, 4)}${item.substring(6, item.length).split('-').join('')}`);
  return Arr;
};
...
const allHappy = (arr) => {
  // Solution code here...
  let happy = false;
  arr.length === 0 ? happy = true : happy = arr.reduce((acc, item) => item.includes(':)') === false ? acc = false : acc , true);
  return happy;
};
