...
function transformToLis(obj){
  // Solution code here...
  let Arr = Object.entries(obj).map(item => `<li>${item.join(': ')}</li>`); 
  return Arr;
};
...
const count = (target, input) => {
  // Solution code here...
  let numOfTimes = input.filter(item => item.includes(target) ? true : false).reduce((idx, item) => idx = item.reduce((ele, number) => target === number ? idx += 1 : idx, 0), 0);
  return numOfTimes;
};
...
const totalSum = (input) => {
  // Solution code here...
  let Sum = input.reduce((idx, item) => idx = item.reduce((ele, number) => idx += number, 0), 0)
  return Sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let Arr = input.map(item => item.filter(item => typeof item === 'number' && item % 5 === 0 ? true : false).map(number => Math.pow(2, number)));
  return Arr;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let Str = [];
  Object.values(data).forEach(item => item.gender === 'male' || item.gender === 'female' ? Str.push(item.name) : false);
  return Str.join(' and ');
};
...
let findShortest = (data) => {
  // Solution code here...
  let Test = data.reduce((idx, item) => idx < item.height ? idx = item.height : idx, 0);
  let shorTest = data.reduce((idx, item) => item.height === Test ? idx = item.name : idx, '');
  return shorTest;
};
